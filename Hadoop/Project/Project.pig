...
inputFile1 = LOAD 'hdfs:///user/asang2024/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile2 = LOAD 'hdfs:///user/asang2024/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile3 = LOAD 'hdfs:///user/asang2024/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
...
STORE ordered INTO 'hdfs:///user/asang2024/output' USING PigStorage('\t');
